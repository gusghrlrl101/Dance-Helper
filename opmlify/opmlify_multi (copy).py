...
def run_single_fit(img,
    ...
    for deg in do_degrees:
        ...
        # hyunho
        print "save 3d" + str(verts.shape[0]) + "vertex to '/result/my.txt'...."
        np.savetxt('result/my.txt', verts)
...
#    openpose_img = datum.cvOutputData
#    cv2.imshow("openpose", openpose_img)
#    cv2.waitKey(0)
...
def smplify(img, out_path, out_video_dir, op_joints, op_confs, cnt, genders, model_female, model_male, use_neutral, sph_regs_female, sph_regs_male, use_interpenetration, n_betas, flength, pix_thsh, viz, do_degrees):
        if exists(out_path):
            out_path = out_path[:-4] + '_new.pkl'

        _LOGGER.info('Fitting 3D body on `%s` (saving to `%s`).', out_video_dir, out_path)
...
def main(base_dir,
    ...
    if not use_neutral:
        ...
        with open('lsp/lsp_gender.csv') as f:
            genders = f.readlines()
    ...
    # Load joints
    est = np.load('lsp/est_joints.npz')['est_joints']

    # hyunho
    # Load video
    if video != "False":
        out_video_dir = '%s/%s' % (out_dir, video[:-4])
        ...
        while cap.isOpened():
            ...
            if not exists(out_path) or cnt in changed:
                if len(procs) < max_proc:
                    procs.append(Process(target=smplify, args=(img, out_path, out_video_dir, op_joints, op_confs, cnt, genders, model_female, model_male, use_neutral, sph_regs_female, sph_regs_male, use_interpenetration, n_betas, flength, pix_thsh, viz, do_degrees)))
    ...
    for ind, img_path in enumerate(img_paths):
        ...
        if not exists(out_path):
            ...
            # openpose
            mystart = time()
            joints, conf = openPose(img)
            print "openpose time: " + str(time() - mystart)
            ...
            params, vis = run_single_fit(
                ...
                viz=viz,
                do_degrees=do_degrees)
            print "run time: " + str(time() - mystart)
...
if __name__ == '__main__':
    ...
    main(args.base_dir, args.out_dir, use_interpenetration, args.n_betas,
         args.flength, args.side_view_thsh, args.gender_neutral, args.viz, args.video)
