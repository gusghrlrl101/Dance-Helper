...
def smplify(img, out_path, out_video_dir, op_joints, op_confs, cnt, genders, model_female, model_male, sph_regs_female, sph_regs_male, n_betas, flength, pix_thsh, do_degrees, female):
        print out_path
        if exists(out_path):
        ...
        for cnt, img in enumerate(frames):
            ...
            if not exists(out_path) or (cnt in changed and not exists(out_path[:-4] + '_new.pkl')):
                if len(procs) < max_proc:
                    procs.append(Process(target=smplify, args=(img, out_path, out_video_dir, op_joints, op_confs, cnt, genders, model_female, model_male, sph_regs_female, sph_regs_male, n_betas, flength, pix_thsh, do_degrees, female)))
